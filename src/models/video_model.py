...
class DMC(CompressionModel):
    ...
    def get_q_for_inference(self, q_in_ckpt, q_index):
        ...
        mv_y_q_enc = self.get_curr_q_enc(mv_y_q_scale_enc, self.mv_y_q_basic_enc, q_index=q_index)
        mv_y_q_scale_dec = self.mv_y_q_scale_dec if q_in_ckpt else self.mv_y_q_scale_dec_fine
        mv_y_q_dec = self.get_curr_q_dec(mv_y_q_scale_dec, self.mv_y_q_basic_dec, q_index=q_index)

        y_q_scale_enc = self.y_q_scale_enc if q_in_ckpt else self.y_q_scale_enc_fine
        y_q_enc = self.get_curr_q_enc(y_q_scale_enc, self.y_q_basic_enc, q_index=q_index)
        y_q_scale_dec = self.y_q_scale_dec if q_in_ckpt else self.y_q_scale_dec_fine
        y_q_dec = self.get_curr_q_dec(y_q_scale_dec, self.y_q_basic_dec, q_index=q_index)
    ...
    def encode_decode(self, x, dpb, q_in_ckpt, q_index, output_path=None,
        ...
        return result
    def decode(self, output_path=None,dpb = None,
