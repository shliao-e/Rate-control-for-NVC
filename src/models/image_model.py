...
class IntraNoAR(CompressionModel):
    ...
    def get_q_for_inference(self, q_in_ckpt, q_index):
        ...
        curr_q_enc = self.get_curr_q(q_scale_enc, self.q_basic_enc, q_index=2)
        q_scale_dec = self.q_scale_dec[:, 0, 0, 0] if q_in_ckpt else self.q_scale_dec_fine
        curr_q_dec = self.get_curr_q(q_scale_dec, self.q_basic_dec, q_index=2)
    ...
    def encode_decode(self, x, q_in_ckpt, q_index,
        ...
        delt_compress_t = compressed_time - t_i
        ...
        decompress_t = time.time()
        delt_decompress_t = decompress_t - compressed_time

        result = {
            'bit': bit,
            'x_hat': x_hat,
            't1':delt_compress_t,
            "t2":delt_decompress_t
        }
        return result


    def decode(self, output_path):
        # pic_width and pic_height may be different from x's size. X here is after padding
        # x_hat has the same size with x
